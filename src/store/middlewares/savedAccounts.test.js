...
describe('SavedAccounts middleware', () => {
  ...
  it('should make a request for the account information, if a relevant transaction was made', () => {
    getAccountStub.expects('getAccount').withArgs(match.any, '1155682438012955434P').returnsPromise().resolves({ balance: 1 });
    const transactions = { transactions: [{ senderId: '1234P', recipientId: '1155682438012955434P' }] };
  ...
  it('should make only one request for the account information, if several transactions for the same account were made', () => {
    getAccountStub.expects('getAccount').withArgs(match.any, '1155682438012955434P').returnsPromise().resolves({ balance: 1 });
    const transactions = {
      transactions: [{ senderId: '1234P', recipientId: '1155682438012955434P' },
        { senderId: '1234P', recipientId: '1155682438012955434P' }],
    };
  ...
  it('should not make a request for the account information, if no relevant transaction was made', () => {
    getAccountStub.expects('getAccount').withArgs(match.any, '1155682438012955434P').returnsPromise().resolves({ balance: 1 });
    const transactions = { transactions: [{ senderId: '1234P', recipientId: '4321P' }] };
    middleware(store)(next)({
      type: actionTypes.newBlockCreated,
      data: { block: transactions },
    });
