...
class Login extends React.Component {
  ...
  onLoginSubmission(passphrase) {
    // const network = this.getNetwork(); 13.250.116.189
    const network = {
      name: 'Custom Node',
      custom: true,
      ...
      <Box className={styles.wrapper}>
        ...
        <section className={`${styles.login} ${styles[this.state.passInputState]}`}>
          <section className={styles.table}>
            <header style={ sectionBackgroundStyle }></header>
            <div ></div>
            <div className={`${styles.tableCell} text-left`}>
              ...
              <form onSubmit={this.onFormSubmit.bind(this)}>
                ...
                <footer className={ `${grid.row} ${grid['center-xs']}` }>
                  ...
                  <div className={grid['col-xs-4']}>
                    <Link className='login-button' to={routes.register.path} refresh="true">
                      <PrimaryButton label={this.props.t('CREATE NEW')}
                        className='login-button' />
                    </Link>
                  </div>
                </footer>
