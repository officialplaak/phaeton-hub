...
describe('Add Account ID Component', () => {
  ...
  beforeEach(() => {
    const store = fakeStore({
      followedAccounts: { accounts: [{ address: '16313739661670634666P', balance: 0 }] },
    });
  ...
  it('accepts valid address', () => {
    wrapper.find('.address input').simulate('change', { target: { value: '11004588490103196952P' } });
  ...
  it('recognizes already following address', () => {
    wrapper.find('.address input').simulate('change', { target: { value: '16313739661670634666P' } });
  ...
  it('goes to next step on button click', () => {
    ...
    expect(props.nextStep).to.have.been.calledWith({ address: '11004588490103196952P' });
  });
});
