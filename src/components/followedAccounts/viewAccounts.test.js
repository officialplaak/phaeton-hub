  ...
  describe('With followed accounts', () => {
    beforeEach(() => {
      ...
      const store = fakeStore({
        followedAccounts: {
          accounts: [
            { address: '123P', balance: 0, title: 'bob' },
            { address: '567P', balance: 100000, title: '' },
          ],
        },
      });
    ...
    it('shows list of followed accounts', () => {
      ...
      expect(wrapper.find('PhaetonAmount').at(0).text()).to.contain(0);

      expect(wrapper.find('.account-title input').at(1)).to.have.value('567P');
      expect(wrapper.find('PhaetonAmount').at(1).text()).to.contain(1);
    });
    ...
    it('edits an accounts title', () => {
      ...
      expect(followedAccounts.followedAccountUpdated).to.have.been.calledWith({
        address: '567P', balance: 100000, title: 'my friend',
      });
    });
  });
});
