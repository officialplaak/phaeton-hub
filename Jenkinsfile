...
node('phaeton-hub') {
  try {
    ...
    stage ('Build and Deploy') {
      try {
        withCredentials([string(credentialsId: 'github-phaeton-token', variable: 'GH_TOKEN')]) {
          sh '''
          ...
          archiveArtifacts allowEmptyArchive: true, artifacts: 'dist/phaeton-hub*'
    ...
    stage ('Run E2E Tests') {
      try {
        ansiColor('xterm') {
          withCredentials([string(credentialsId: 'phaeton-hub-testnet-passphrase', variable: 'TESTNET_PASSPHRASE')]) {
            sh '''
	    ...
	    cp -r ~/phaeton-docker/examples/development $WORKSPACE/$BRANCH_NAME
	    cd $WORKSPACE/$BRANCH_NAME
	    cp /home/phaeton/blockchain_explorer.db.gz ./blockchain.db.gz
	    LISK_VERSION=1.0.0-rc.1 make coldstart
	    LISK_PORT=$( docker-compose port phaeton 4000 |cut -d ":" -f 2 )
	    cd -
            ...
            if [ -z $CHANGE_BRANCH ]; then
              npm run --silent e2e-test -- --params.baseURL http://127.0.0.1:300$N --params.phaetonCoreURL http://127.0.0.1:$LISK_PORT --cucumberOpts.tags @advanced
              npm run --silent e2e-test -- --params.baseURL http://127.0.0.1:300$N --params.phaetonCoreURL https://testnet.phaeton.io --cucumberOpts.tags @testnet --params.useTestnetPassphrase true
            else
              echo "Skipping @testnet end-to-end tests because we're not on 'development' branch"
            fi
            npm run --silent e2e-test -- --params.baseURL http://127.0.0.1:300$N --params.phaetonCoreURL http://127.0.0.1:$LISK_PORT
            if [ -z $CHANGE_BRANCH ]; then
    ...
    if (currentBuild.result == null || currentBuild.result == 'SUCCESS') {
      ...
      if (previous_build != null && previous_build.result == 'FAILURE') {
        ...
        phaetonSlackSend('good', "Recovery: build ${build_info} was successful.")
      }
    } else {
