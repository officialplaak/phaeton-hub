...
import * as phaetonServiceApi from '../../src/utils/api/phaetonService';
...
class Helper extends GenericStepDefinition {
  ...
  checkConfirmRequest(account) {
    ...
    this.haveTextOf('.copy-title', `phaeton://wallet?recipient=${address}&amount=1`);
    expect(this.wrapper.find('QRCode')
      .filterWhere(item => item.prop('value') === `phaeton://wallet?recipient=${address}&amount=1`))
      .to.have.lengthOf(1);
  }
...
describe('@integration: Wallet', () => {
  ...
  beforeEach(() => {
    ...
    phaetonServiceStub = stub(phaetonServiceApi, 'getPriceTicker');
    phaetonServiceStub.withArgs(match.any).returnsPromise().resolves({ tickers: {} });
  });
  ...
  describe('Send', () => {
    ...
    describe('Scenario: should not allow to send when not enough funds', () => {
      ...
      step('And I fill in "537318935439898807L" to "recipient" field', () => helper.fillInputField('537318935439898807P', 'recipient'));
    ...
    describe('Scenario: should give and error message when sending fails', () => {
      ...
      step('When I click "send button"', () => {
        sendTransactionsStub.withArgs(match.any, '537318935439898807P', match.any, accounts.genesis.passphrase, match.any, match.any).returnsPromise().rejects({});
    ...
    describe('Scenario: should show account initialisation option if no public key and balance is greater than 0', () => {
      ...
      step('Then I should see the account init option', () => helper.haveTextOf('header h2', 'Initialize Phaeton ID'));
      step('When I click "account init button"', () => helper.clickOnElement('.account-init-button button'));
      step('Then I should be on the confirm page', () => helper.haveTextOf('header h2', 'Initialize Phaeton ID'));
      step('When I click "send button"', () => helper.clickOnElement('button.send-button button'));
      step(`Then I should see text ${successMessage} in "result box message" element`, () => helper.haveTextOf('.result-box-message', successMessage));
    });
