...
describe('MenuBuilder', () => {
  ...
  beforeEach(() => {
    electron = {
      ...
      app: { getName: () => ('Phaeton Hub'), getVersion: () => ('0.2.0') },
  ...
  it('Builds the electron about menu when os is mac', () => {
    ...
    expect(template[0].label).to.equal('Phaeton Hub');
  ...
  it('Builds the electron about menu when os is not mac', () => {
    ...
    expect(template[0].label).to.not.equal('Phaeton Hub');
  ...
  it('Should open link on click', () => {
    menu.onClickLink(electron, 'https://phaeton.io');
    expect(electron.shell.openExternal).to.have.been.calledWith('https://phaeton.io');
  });
});
